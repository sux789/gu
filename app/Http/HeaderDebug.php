...
class HeaderDebug
{
    ...
    public static function log($label, $results = 'Temporary Value', $caller = '')
    {
        ...
        if ($results === 'Temporary Value') {
            array_push(self::$logTable, array('[临时调试]', $label, $caller));
        } else {
            if (is_string($results) && mb_detect_encoding($results, 'UTF-8') !== 'UTF-8') {
                $results = '非UTF-8编码，长度：' . strlen($results);
            }
            array_push(self::$logTable, array($caller, $label, $results));
        }
    }


    static function getLogAt($input)
    {
        $delimiter = '/';
        $file = '';
    ...
    public static function show()
    {
        ...
        if ($count = count(self::$logTable)) {
            array_unshift(self::$logTable, ['代码所在文件名:行数', 'Label', 'debug变量结果']);
